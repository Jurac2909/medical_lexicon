...
class MedicalNERApp:
    def __init__(self, root: tk.Tk) -> None:
        self.root = root
        self.log = get_logger()
        self.analyzer: Analyzer = MedicalNERAnalyzer()
        self.fetcher: DescriptionFetcher = TermInfoFetcher()
    ...
    def _build_ui(self) -> None:
        ...
        tk.Label(
            header,
            text=(
                "Detects diseases, symptoms, therapies, diagnostics "
                "and anatomy using a neural network (NER)."
            ),
            bg=ACCENT,
            fg="#d6e6f0",
            font=("Segoe UI", 9),
        ...
        ttk.Label(
            main, text="Input text", font=("Segoe UI", 10, "bold")
        ).pack(anchor="w")
        ...
        ttk.Label(
            main,
            text=(
                "ℹ  Term definitions are not shown here — they are "
                "included in the CSV and PDF exports."
            ),
            foreground="#6b7a86",
        ...
        ttk.Label(
            main,
            textvariable=self.results_header,
        ...
        self.tree.column(
            "category", width=160, anchor="center", stretch=False
        )
        self.tree.column(
            "confidence", width=130, anchor="center", stretch=False
        )
    ...
    def on_analyze(self) -> None:
        ...
        if not text:
            messagebox.showwarning(
                "Warning", "Please enter text to analyze."
            )
            return

        self._set_busy(True, "Loading model and analyzing text...")
        worker = threading.Thread(
            target=self._analyze_worker, args=(text,), daemon=True
        )
        worker.start()
    ...
    def _show_results(self, terms: list[MedicalTerm]) -> None:
        ...
        self._set_busy(
            False,
            f"Done. Found {len(terms)} terms. "
            "Export to save definitions.",
        )
        if not terms:
            messagebox.showinfo(
                "Result", "No medical terms were found in the text."
            )
    ...
    def on_export(self, fmt: str) -> None:
        if not self.terms:
            messagebox.showinfo(
                "Export",
                "No results to export. Analyze some text first.",
            )
            return
